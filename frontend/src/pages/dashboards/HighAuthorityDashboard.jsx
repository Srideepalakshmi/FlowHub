...
const HighAuthorityDashboard = () => {
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 transition-colors duration-300 p-6 rounded-2xl shadow-sm border border-blue-100 dark:border-zinc-800 max-w-5xl mx-auto">
      ...
      <div className="bg-purple-50 dark:bg-purple-900/10 border border-purple-200 dark:border-purple-800/50 rounded-xl p-6 flex justify-between items-center mb-8 transition-colors">
         <div>
         ...
         <button onClick={fetchPending} className="bg-purple-600 dark:bg-purple-500 text-white px-4 py-2 rounded-lg font-medium hover:bg-purple-700 transition">
           Refresh Inbox
         </button>
      </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-slate-200 dark:border-zinc-800">
                <th className="pb-3 text-sm font-semibold text-slate-500 dark:text-slate-400">Requester</th>
                <th className="pb-3 text-sm font-semibold text-slate-500 dark:text-slate-400">Workflow Request</th>
                <th className="pb-3 text-sm font-semibold text-slate-500 dark:text-slate-400">Target Date</th>
                <th className="pb-3 text-sm font-semibold text-slate-500 dark:text-slate-400 flex justify-end">Final Action</th>
              </tr>
            </thead>
            <tbody className="text-sm">
              {executions.length === 0 ? (
                <tr><td colSpan="4" className="py-8 text-slate-500 text-center">Your queue is empty.</td></tr>
              ) : null}
              {executions.map(ex => (
                <tr key={ex.id} className="border-b border-slate-100 dark:border-zinc-800 hover:bg-slate-50 dark:hover:bg-zinc-800/50 transition-colors">
                  <td className="py-4 font-medium text-slate-800 dark:text-slate-200">{ex.triggered_by}</td>
                  <td className="py-4">
                    <span className="text-slate-600 dark:text-slate-400 block font-medium">{ex.workflow_name}</span>
                  ...
                  <td className="py-4 text-slate-600 dark:text-slate-400">{new Date(ex.started_at).toLocaleDateString()}</td>
                  <td className="py-4 flex justify-end gap-3">
                    <button onClick={() => handleAction(ex.id, 'Approve')} className="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 border border-green-200 dark:border-green-800 px-4 py-1.5 rounded hover:bg-green-200 transition font-medium">Approve</button>
                    <button onClick={() => handleAction(ex.id, 'Reject')} className="bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 border border-red-200 dark:border-red-800 px-4 py-1.5 rounded hover:bg-red-200 transition font-medium">Reject</button>
                  </td>
                </tr>
              ))}
            </tbody>
